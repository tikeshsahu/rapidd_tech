...
class Task {
  ...
  Task({required this.id, required this.title, required this.description, required this.timestamp, required this.toBeSeenBy});
  ...
  factory Task.createNew(String title, String description) {
    return Task(id: '${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(10000)}', title: title, description: description, timestamp: Timestamp.now(), toBeSeenBy: [StorageService.instance.fetch(AppUtils.userEmail)]);
  }
}
